...
ctrl+c: 复制  
ctrl+shift+c:复制为路径  
选择一个文件，ctrl+c复制，ctrl+shift+c复制文件路径。  

alt+方向键：alt+←/alt+→,后退/前进，网页浏览/文件夹导航/福昕pdf阅读器等可以用。  

backspace键：返回文件夹的上一级  

ctrl+shift+esc = ctrl+alt+del: 调出任务管理器  

alt+f4:  
关闭当前程序，关机。在当前程序界面上按alt+f4关闭当前程序，在桌面上按alt+f4关机。  
...
shift+insert，粘贴。word/cygwin/notepad等软件均支持这个快捷键。  
win+shift+s，截屏，windows11系统自带。  
windows+方向键：调整窗口大小和位置  
windows+print screen： 全屏截屏  
ctrl+shift+t： 重新打开关闭的浏览器页面  
ctrl+y： 重做撤销的操作  
windows+p ： 变更多屏幕显示模式  

windows+**.** ：  
打开表情符号，win+**;**功能与此一样。特殊符号、粘贴板也在这里。  

windows+v ： 打开剪贴板历史记录  
win+ctrl+o：打开屏幕键盘。与osk命令功能一样。键盘坏了或远程桌面按特殊键时有用。  

win+k，无线投屏，miracast。  

windows+数字：快速启动任务栏上的应用，比鼠标点击操作快多了  
win+1‌ 和 ‌win+2‌ 是 windows 系统中用于快速启动或切换任务栏上应用程序的快捷键。  
-   ‌win+1‌：打开或切换到‌任务栏从左到右第1个位置‌的应用程序(如浏览器、微信等)。  
-   ‌win+2‌：打开或切换到‌任务栏从左到右第2个位置‌的应用程序。  
如果该应用已运行，则直接切换到其窗口；若未运行，则会启动该应用。  
补充说明  
-   任务栏图标顺序以‌固定顺序为准‌，不受当前是否打开影响。  
-   可配合修饰键实现更多功能：  
    -   ‌win + shift + 数字‌：以新实例打开该应用(如多开浏览器)‌‌。  
    -   ‌win + ctrl +  
        数字‌：切换到该应用的最后一个活动窗口(适用于多窗口应用)‌‌。  
    -   ‌win + alt + 数字‌：打开该应用的跳转列表(最近文件或网站)‌‌。  
...
calc命令，计算器，键盘esc键是计算器的CE键:清除，backspace是逐个字符删除。  
...
支持命令行：   
Snipaste snip                           //打开默认截图  
Snipaste snip --custom                  //指定区域截图  
Snipaste snip --full                    //全屏截图  
Snipaste snip -o D:\img\capture.png     //截图并保存到文件  
Snipaste snip -o clipboard              //截图，并复制到粘贴板  
Snipaste snip --delay 5                 //延迟5秒截图  
Snipaste snip --size 800 600            //指定尺寸，宽800、高600  
Snipaste snip --size 1920 1080 -o D:\img\shot.png  //综合  
...
方法三：更快的方法，打开所在文件夹，在路径上输入cmd、回车，弹出cmd命令窗口，瞬间出现，路径且为当前路径。 -- 打开经典cmd窗口、启动非常快。  
步骤：  
1.打开文件夹，进入到需要的目录， -- 文件夹浏览器、也称资源管理器  
![打开文件夹，进入到需要的目录](../Resources/打开文件夹，进入到需要的目录.png)  
2.地址栏输入cmd、回车  
![地址栏输入cmd、回车1](../Resources/地址栏输入cmd、回车1.png)  -> ![地址栏输入cmd、回车2](../Resources/地址栏输入cmd、回车2.png)  
打开cmd窗口，并且进入到当前所在路径，如下，  
![打开cmd窗口，并且进入到当前所在路径](../Resources/打开cmd窗口，并且进入到当前所在路径.png)  
...
#### netsh命令
###### netsh命令
netsh interface ip show interfaces  //查看网卡  
netsh interface ip show address     //查看IP  
netsh interface ip show address "以太网 2"  //查看指定网卡IP  
netsh interface ip set address name="本地连接" source=dhcp  //设置为dhcp  
netsh interface ip set address name="本地连接" source=static addr=192.168.0.106 mask=255.255.255.0  //设置为静态ip  
netsh interface ip set address "以太网" static 192.168.1.100 255.255.255.0 192.168.1.1  

**netsh wlan**  
netsh wlan show interfaces //查看无线网口  
netsh wlan show interface name="name" //查看指定无线网口  
netsh wlan show networks //显示计算机上可见的网络列表  
netsh wlan show networks \| findstr /i ssid  
netsh wlan show wlanreport //生成无线网络报告，很详细，可用于bug定位  

netsh wlan show profile //显示本机保存的profile,配置文件是以wifi的ssid命名的  
netsh wlan show profile interface="name" //查看指定网卡的profile  
netsh wlan show profile name="profile" //查看指定profile的内容，profile写对应网络的ssid   
netsh wlan show profile name="profile" key=clear //查看指定profile的内容，查看保存的无线网络密码  
netsh wlan connect name="profile" //连接到无线网络  
netsh wlan connect "profile" //不需要写name=  
netsh wlan connect ssid=ssid name="profile"  
netsh wlan connect ssid=ssid name="profile" interface="interface" //电脑中存在多个网卡则指定interface  
netsh wlan disconnect //断开连接  
netsh wlan delete profile name="profile" //删除配置文件  
netsh wlan delete profile profile //不需要写name=  

netsh interface set interface "interface"enabled //开启无线接口  
netsh interface set interface "interface"disabled //关闭无线接口   
netsh wlan set profileparameter name="profile" connectionmode=manual //禁用自动连接  
netsh wlan set profileparameter name="profile" connectionmode=auto  


###### 切换网卡上网
windows11有时需要用有线连接网络、有时需要用WiFi连接网络，有什么方法可以快速实现有线、无线的快速切换？  
1.禁用/启用有线网络（假设你的有线网卡名称为“以太网”）：  
o禁用命令：netsh interface set interface "以太网" disable  
