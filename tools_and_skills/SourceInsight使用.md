...
ctrl+m ： 书签  
alt+m ： 在最近浏览过的两个位置之间切换显示，方便来回查看对比(可以在文件内切换，也可以在文件之间切换)。  
ctrl+shift+m ： 打开“浏览历史记录”弹出框。在最近浏览过的多个位置之间切换显示。  

ctrl+g ： 跳到某一行  
ctrl+z ： 撤销修改  
ctrl+y ： 恢复修改  

ctrl+鼠标左击,或ctrl+= ： 跳到函数/宏/变量定义(声明)。  
ctrl+鼠标右击,或alt+/ ： 弹出对话框，弹出symbol info框,显示函数/宏/变量定义。按esc关闭弹出对话框。  
esc键 ： 关闭弹出对话框，取消当前任务，关闭当前窗口，中止、停下现在的工作。  

ctrl+f ： 查找  
f3 ： 向后搜索  
f4 ： 向前搜索  
shift+f3 ： 向后搜索鼠标所在位置的单词  
shift+f4 ： 向前搜索鼠标所在位置的单词  
shift+f3/shift+f4更快捷，直接搜索鼠标所在位置的字符串，省去了ctrl+f的多个步骤 ： 弹出搜索框、输入字搜索符串、按确认开始搜索。  
取消循环搜索：ctrl+f,弹出搜索框,将Wrap Around选项去掉。  

ctrl+shift+f ： 在多个文件中搜索  
或者ctrl+f然后选择Files,在弹出的Search files窗口选择需要搜索的文件夹或者文件。  
当要查询的文件夹包含子文件夹时，可以勾选Options中的IncludeSubdirectories，实现对各层文件的递归搜索。指定目录搜索，冗余搜索耗费时间。  
备注 ： 查找/替换,记住ctrl+fctrl+h就可以了，如果要在指定目录或多个文件查找/替换，按ctrl+fctrl+h后，选择Files..选择目录或文件。不要记ctrl+shift+f/ctrl+shift+h。  

ctrl+h ： 替换  
ctrl+shift+h ： 多文件替换  

ctrl+/ ： Lookup Reference，搜索(状态工程)。  

菜单栏有一个带问号的图标Active search results,打开搜索结果。  
a.在搜索结果页面，按ctrl+l(字母l)跳到搜索结果对应的代码位置，再按一次回到搜索结果的位置  
b.在搜索结果页面，点击工具栏 go to next link(shift+f8)/go to next link(shift+f9)，跳转到鼠标所在搜索结果对应的代码位置，再次点击可以在跳转到下一个/上一个搜索结果对应代码的位置。类似页面搜索f3/f4。  

alt+/ ： 符号信息(symbol info)，displays a popup window showing a symbol's definition  

alt+1 ： 符号索引，激活符号检索框，检索当前文件内的符号。  

f7/alt+g ： 符号索引，激活符号检索框，检索整个工程中的符号。可以搜索变量、函数、宏定义等等。  
...
打乱名字中各字符的次序也能检索到，例如，输入authe sme或者 send sme aut等，都能检索到sme_send_authentication()函数。  

ctrl+o ： 打开文件列表框，注意并不是打开文件，这个快捷键和其他软件不一样，其他软件ctrl+o是打开文件。  

ctrl+shift+o ： 重新加载文件(如果文件在其他编辑器里被修改,sourceinsight还没有同步更新,则按ctrl+shift+o手动重新加载文件)  

ctrl+w ： 关闭文件。  
ctrl+shift+w ： 关闭所有文件  

ctrl+shift+\[ ： 光标跳到块(大括号)的下面  
ctrl+shift+\] ： 光标跳到块(大括号)的上面  

shift+f8 ： 高亮当前单词。按一次高亮，按两次取消高亮。  
f8 ： source insight4高亮当前单词改成了f8  
alt+f8 ： 显示/隐藏函数列表窗口  

ctrl+shift+m ：   
打开“选择历史”(SelectionHistory)窗口，用于查看和切换最近一次或多次的文本选择记录。  
当你在代码中进行了多次不同的文本选择(如选中变量、函数名、代码块等),可通过ctrl+shift+m快速回顾并重新定位到之前的某个选择位置。有助于在复杂代码中高效地在多个选区之间切换，提升导航效率。  


alt快捷键  
alt+f ： 打开file下拉菜单栏，f<-file  
alt+e ： 打开edit下拉菜单栏，e<-edit  
alt+s ： 打开search下拉菜单栏，s<-search  
alt+w ： 打开window下拉菜单栏  
小结 ： alt+字母，字母为菜单栏名字的首字母(其他软件也一样)。  

菜单→Window→ Pick Window，弹出框显示当前打开的文件列表。\/\/si3.5中是Window List  
菜单→File→RecentFiles，打开最近打开的文件列表。如果菜单→Window中对应的文件已经关闭了，还可以通过菜单→File→ Recent Files打开。  

数字小键盘区-上一个函数，需要取消小键盘NumLock才可以使用-/+进行跳转。  
数字小键盘区+下一个函数  
alt+数字小键盘区- ： 前一个修改,需要取消小键盘NumLock才可以使用alt+/+进行跳转。  
alt+数字小键盘区+ ： 下一个修改  

其他  
alt+0 ： 跳到基本类型(即跳到原型)  
shift+f9或ctrl+shift+l ： 转到下一个链接  
alt++/alt+→/alt+↑/alt+↓ ： alt+方向键，不移动鼠标指针、移动页面视图  
...
shift+end      ：从当前位置选择到行结束  
shift+home     ：从当前位置选择到行的开始  
ctrl+shift+home ：从当前位置选择到文件顶部  

按住alt，按住鼠标左键拖动，选中一个矩形区块(多行多列的一部分区域)，然后进行复制/粘贴/编辑。  
...
ctrl+shift+m / ctrl+shift+z，配置为“浏览历史记录”。  
注：alt+z/alt+x、f1/f12配置为快捷键**方便左右手都可以使用**；将软件自带shift+f8/shift+f9快捷键删除；shift+f8配置为清除所有语法高亮，方便高亮字符太多的时候一次性清除。` 
...
si3.5 ：  C ： \Users\lenovo\Documents\Source Insight\Projects  
si4.0 ：  C ： \Users\lenovo\Documents\Source Insight 4.0\Projects  
...
2.注意：sourceinsight中`\w`不代表所有单个大小写字母、数字、下划线，不表示`[a-zA-Z0-9_]`, sourceinsight中`\w`表示空格或tab，\w-white；而python/perl中`\w`表示单个大小写字母、数字、下划线，`[a-zA-Z0-9_]`，\w-word。  
...
3.不支持欲搜索、不支持竖线`|`、`\b`、`\<`、`\>`等，支持哪些正则？参考下一节 ： 1.si支持的正则表达式  
...
`n`  new-line, or the end-of-line charac­ter. Use this if you want to match a line break in the middle of a pat­tern.  
...
`.` `*`  这条RE可以匹配任意的字符串序列，因为`.`示任意字符，`*`表示大于等于0个，连起来理解就是&quot;任意个任意字符&quot;  
