...
ls例举当前目录下所有文件，通过管道|输出给xargs，xargs从管道|读取ls的输出，如filename，和rm \*.o组建成命令，如rm filename \*.o，然后执行命令，删除filename文件、和\*.o文件。由于ls例举了当前目录下所有文件，因此ls | xargs rm \*.o执行的结果就是删除当前目录下所有文件。
