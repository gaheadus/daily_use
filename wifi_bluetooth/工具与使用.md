...
#### 过滤条    
过滤条，其实就是将之前使用鼠标点击使用过滤器的操作，转化为纯文本的操作。
过滤条可以使用表达式，可以实现比过滤器更复杂的操作，可以对显示结果进行更精确的控制。

**1.示例**  
filter('test')  
filter('doing')  
pspec('802.11 auth') | pspec('802.11 assoc req') | pspec('802.11 assoc rsp') | pspec('802.1x') | pspec('dhcp') | pspec('arp')  
...
802.11 beacon、802.11 probe req、802.11 probe rsp、802.11 auth、802.11 deauth、802.11 assoc req、802.11 assoc rsp、802.11 disassoc、802.11 reassoc req、802.11 reassoc rsp、802.11 addba req、802.11 addba rsp、……  
