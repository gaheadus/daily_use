...
## wireshark  
[wireshark过滤](#wireshark过滤)  &emsp;&emsp;  [wireshark怎么抓包](#wireshark怎么抓包)  &emsp;&emsp;  [wireshark常用选项与功能总结](#wireshark常用选项与功能总结)    

### wireshark过滤  
wireshark选中一个bit或字段，右击，弹出对话框：  
1.作为过滤器应用 -- 过滤对应的包  
2.用过滤器着色 -- 对应的包着色  
3.准备过滤器 -- 在过滤器框自动填写对应的过滤器，这样就不需要记住具体的过滤器名字和对应的值了  
