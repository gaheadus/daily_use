...
备注：`RILSIM2.*CGATT`和`(?<=RILSIM2).*CGATT`的搜索结果一样的，是不是没必要使用预搜索？ 不是的，虽然两者结果上一样，但是原理上不一样，`RILSIM2.*CGATT`搜索的是`RILSIM2.*CGATT `，而`(?<=RILSIM2).*CGATT`搜索的是`.*CGATT，(?<=RILSIM2)`只是作为一个附件条件，这个附加条件本身并不参与搜索。从效率上看，预搜索`(?<=RILSIM2).*CGATT`比`RILSIM2.*CGATT`高的多，大文件搜索时更明显。  
